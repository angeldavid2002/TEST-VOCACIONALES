...
def get_users_vocations_csv_service():
    ...
    try:
        ...
        ).join(VocacionDeUsuarioPorTest, Usuario.id == VocacionDeUsuarioPorTest.id_usuario).all()
        output = io.StringIO()
...
def get_cities_common_vocation_csv_service(current_user: dict):
    db = next(get_db_session())
    try:
        # Consulta para obtener ciudades con la vocación más común
        results = db.query(
        ...
        ).group_by(Ciudad.id, Ciudad.nombre, Ciudad.latitud, Ciudad.longitud, VocacionDeUsuarioPorTest.moda_vocacion
        ).all()
        output = io.StringIO()
        writer = csv.writer(output,delimiter=";")
...
def get_vocation_percentages_csv_service(current_user: dict):
    ...
    try:
        ...
        # Obtener vocación y el conteo de registros para cada vocación
        results = db.query(
            VocacionDeUsuarioPorTest.moda_vocacion,
...
def get_users_by_city_csv_service(current_user: dict):
    ...
    try:
        ...
        ).join(Usuario, Ciudad.id == Usuario.id_ciudad).all()
        output = io.StringIO()
        writer = csv.writer(output,delimiter=";")
