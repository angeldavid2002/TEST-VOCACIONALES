...
@router.post("/register", response_model=model_user.Usuario)
def register(usuario: model_user.UsuarioCreate, db: Session = Depends(get_db)):
...
@router.post("/token")
async def login(form_data: model_user.UsuarioCreate, db: Session = Depends(get_db)):
